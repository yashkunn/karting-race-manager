...
class RegisterForRaceViewTests(TestCase):
    ...
    def test_get_registration_form(self):
        ...
        response = self.client.get(reverse("karting:register-for-race", args=[self.race.id]))
    ...
    def test_register_for_race_success(self):
        self.client.login(username="testuser", password="password")
        response = self.client.post(reverse("karting:register-for-race", args=[self.race.id]), {
            "kart": self.kart.id,
        })

        self.assertRedirects(response, reverse("karting:race-detail", args=[self.race.id]))
        self.assertTrue(RaceParticipation.objects.filter(user=self.user, race=self.race).exists())
    ...
    def test_register_for_race_already_registered(self):
        self.client.login(username="testuser", password="password")
        RaceParticipation.objects.create(user=self.user, race=self.race, kart=self.kart)
        ...
        self.assertContains(response, "You are already registered for this race.")
    ...
    def test_register_for_race_full(self):
        ...
        self.assertTrue(any("This race is full." in str(msg) for msg in messages))
    ...
    def test_no_registration_button_for_anonymous_user(self):
        ...
        expected_message = 'You must be <a href="/accounts/login/">logged</a> in to register for the race.'
